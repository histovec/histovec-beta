...
export const generateReportRoute = ({ path, logLabel, payloadSchema }) => {
  return {
    ...
    handler: async (request, h) => {
      ...
      const anonymizedNom = nom ? anonymize(nom, 1) : nom
      const anonymizedPrenom = (prenoms && typeImmatriculation === TYPE_IMMATRICULATION.SIV) ? anonymize(prenoms[0], 1) : prenoms
      const anonymizedRaisonSociale = raisonSociale ? anonymize(raisonSociale, 1) : raisonSociale
      const anonymizedSiren = siren ? anonymize(siren, 1) : siren
      const anonymizedNumeroImmatriculation = numeroImmatriculation ? anonymize(numeroImmatriculation, 1) : numeroImmatriculation
      const anonymizedNumeroFormule = numeroFormule ? anonymize(numeroFormule, 1) : numeroFormule
      const anonymizedDateEmissionCertificatImmatriculation = dateEmissionCertificatImmatriculation ? anonymize(dateEmissionCertificatImmatriculation.toString(), 5) : dateEmissionCertificatImmatriculation
      syslogLogger.info({ key: 'payload_data', tag: logLabel, value: { alreadyHasIdAndKey, anonymizedNom, anonymizedPrenom, anonymizedRaisonSociale, anonymizedSiren, anonymizedNumeroImmatriculation, anonymizedNumeroFormule, anonymizedDateEmissionCertificatImmatriculation, id, key } })
      ...
      const normalizedControlesTechniques = normalizeControlesTechniques(anonymizedControleTechnique)

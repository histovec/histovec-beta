...
module.exports.UTACClient = class UTACClient {
  constructor() {
	...
	async _resetAuthenticate() {
		...
		await redis.setAsync('UTAC_AUTH_TOKEN', '')
		this.axios.defaults.headers.common['Authorization'] = null;
		await this._authenticate()
	}

  async _getAuthenticationCache() {
		return await redis.getAsync('UTAC_AUTH_TOKEN')
  }

  async _setAuthenticationCache(authToken) {
		await redis.setAsync('UTAC_AUTH_TOKEN', authToken)
  }
  ...
  async initialize() {
	...
	checkInitialization() {
		if (!this.isInitialized)
			throw new Error('You should call UTACClient.initialize() before using this method');
	}
	...
	async readControlesTechniques(plaque, resetAuthentication=false) {
		...
		try {
			...
			if (error.response && error.response.status === 401) {
				...
				if (!resetAuthentication) {
					return await this.readControlesTechniques(plaque, resetAuthentication=true)
				}
